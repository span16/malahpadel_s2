...
class ProduitController extends AbstractController
{
    ...
    public function editBackOffice(Request $request, Produit $produit, EntityManagerInterface $entityManager, SluggerInterface $slugger): Response
    {
        $form = $this->createForm(ProduitType::class, $produit);
        $form->handleRequest($request);
        $oldImage = $produit->getImageProduit();
        ...
        return $this->render('back/produitback.html.twig', [
            ...
            'produits' => null,
            'total_produits' => 0, // Pas de liste, donc total à 0
        ]);
    }

    #[Route('/admin/delete/{id_produit}', name: 'app_produit_delete2', methods: ['POST'])]
    public function delete2(Request $request, Produit $produit, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete'.$produit->getIdProduit(), $request->request->get('_token'))) {
            $image = $produit->getImageProduit();
            if ($image) {
